...
bool flag_write_numerical_boundary = false;
bool flag_write_domain_boundary = false;
...
void HDF5H::write_grid(T_grptr &grid_ptr, int level_index, hid_t &group0_id, std::string &ini_group)
{
	...
	// char amr_nodes_file_name[1024];
	// sprintf(amr_nodes_file_name, "amr_pointCloude_level%d.vtk", ilevel);
	// amr_nodes_file.open(amr_nodes_file_name, std::ios::out);
	// amr_nodes_file << "# vtk DataFile Version 2.0\n\n";
	// amr_nodes_file << "ASCII\nDATASET POLYDATA\n";
	...
	for (auto iter = grid_ptr.grid.begin(); iter != grid_ptr.grid.end(); ++iter)
	{
		...
		{
			++icount_cells;
			// amr_nodes_file << xyz[icount_points*C_DIMS] << " " << xyz[icount_points*C_DIMS + 1] << " " << xyz[icount_points*C_DIMS + 2] << "\n";
		}
#endif
		++icount_points;
	}
	...
	// amr_nodes_file << "POINTS " << icount_points << " float\n";
	// for (unsigned int iipoint = 0; iipoint < icount_points; ++iipoint) {
	// 	amr_nodes_file << xyz[iipoint*C_DIMS] << " " << xyz[iipoint*C_DIMS + 1] << " " << xyz[iipoint*C_DIMS + 2] << "\n";
	// }
	// amr_nodes_file << "VERTICES " << icount_points << " " << 2*icount_points << "\n";
	// for (unsigned int iipoint = 0; iipoint < icount_points; ++iipoint) {
	// 	amr_nodes_file << "1 " << iipoint << "\n";
	// }
	...
	// std::cout << "level_index " << level_index << " dims2[0] " << dims2[0] << std::endl;
