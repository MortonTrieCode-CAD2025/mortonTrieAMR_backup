...
void Morton_Assist::compute_ref(unsigned int ilevel)
{
	...
	morton_xyz.at(ilevel).set(Morton_Assist::bit_otherlevel - ilevel * C_DIMS, true); //x
	morton_xyz.at(ilevel).set(Morton_Assist::bit_otherlevel - ilevel * C_DIMS + static_cast<long long>(1), true); //yx
#if (C_DIMS==3)
	morton_xyz.at(ilevel).set(Morton_Assist::bit_otherlevel - ilevel * C_DIMS + static_cast<long long>(2), true); //zyx
#endif
...
inline D_morton Morton_Assist::find_neighbor(const D_morton key, const int level, const int x_shift, const int y_shift)
{
	D_morton ngbr_code = key;
	if (x_shift == 1) {
		ngbr_code = Morton_Assist::find_x1(key, level);
	}
	else if (x_shift == -1) {
		ngbr_code = Morton_Assist::find_x0(key, level);
	}
	
	if (y_shift == 1) {
		ngbr_code = Morton_Assist::find_y1(ngbr_code, level);
	}
	else if (y_shift == -1)
	{
		ngbr_code = Morton_Assist::find_y0(ngbr_code, level);
	}
...
void Morton_Assist::compute_coordinate(D_morton key, unsigned int ilevel, D_real &x, D_real &y, D_real &z)
{
	D_real x_temp = C_domain[0];
	D_real y_temp = C_domain[1];
	D_real z_temp = C_domain[2];
	for (unsigned int i = 1; i < ilevel + 1; ++i)
	{
		if (key.test(Morton_Assist::bit_otherlevel - i * C_DIMS))
		{
...
// inline D_morton Morton_Assist::find_neighbor(const D_morton& key, const unsigned int level, const int x_shift, const int y_shift, const int z_shift)
// {
