...
#include <unordered_set>  // Still use unordered_set for D_setLat
#endif
...
public:
	...
	/**
	 * @attention  
	 *            The original LBM module based on this AMR framework may follow the vertex-center LBM.
	 */
...
using D_setLat = std::unordered_set<D_morton>;
#elif(C_MAP_TYPE == 2)
...
using D_setLat = std::set<D_morton>;
#elif(C_MAP_TYPE == 3)
template <typename value>
using D_map_define = MortonTrieMap<value>;
using D_setLat = std::unordered_set<D_morton>;  // Keep using unordered_set for sets
#endif
