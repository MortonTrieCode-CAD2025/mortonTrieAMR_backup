...
public:
	...
	// The set records finest-level nodes close to the solid points, and the amount of solid points close to the finest-level nodes
	//          o-----o
	//         /     /|
...
public:
	...
	const std::array<int, 3> flag_interface = {-1, -2, -3}; // nodes on the numerical boundary;
...
// the bit represents (z1 z0 y1 y0 x1 x0)
	...
	unsigned int ioverlap = 1;  // the order of the overlapped boudary layer from inner to outer at the coarse level (ilevel-1)
